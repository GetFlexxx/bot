...
def load_points():
    """Lädt die Punkte aus der Datei."""
    if os.path.exists(POINTS_FILE):
        with open(POINTS_FILE, "r") as file:
            game_state["points"] = json.load(file)
    else:
        game_state["points"] = {}


def save_points():
    """Speichert die Punkte in die Datei."""
    with open(POINTS_FILE, "w") as file:
        json.dump(game_state["points"], file)
...
async def guess_number(ctx, zahl: int):
    ...
    if zahl < game_state["number"]:
        ...
        rate_logger.warning(
            f"{ctx.author.name} hat die Zahl {game_state['number']}, nach {game_state['attempts']} Versuchen erraten.")

        # Punkte berechnen und vergeben
        points = calculate_points(game_state["attempts"])
        update_points(ctx.author.name, points)
        save_points()  # Speichere Punkte nach jedem Spiel
...
async def end_game_due_to_timeout(ctx):
    ...
    if game_state["active"]:
        ...
        rate_logger.warning(f"{ctx.author.name} hat die Zahl {game_state['number']} in {game_state['attempts']} Versuchen nicht erraten.")
        await reset_game()
...
# Punkte laden beim Importieren des Moduls
load_points()
